...
const navSections = [
  {
    ...
    items: [
      ...
      { id: "gps-tracking", label: "GPS Tracking", icon: Bus },
      { id: "pickup-drop", label: "Pickup / Drop", icon: UserCheck },
    ],
  },
  {
    label: "Operations",
    items: [
      { id: "emergency-alert", label: "Emergency Alert", icon: AlertTriangle },
      { id: "maintenance-log", label: "Maintenance Log", icon: Wrench },
    ],
  },
];
  ...
  const Sidebar = () => (
    <div className="flex flex-col h-full">
      <div className="px-4 py-5 border-b border-sidebar-border">
        ...
        {(driverName || schoolName) && (
          <div
            className="mt-3 px-2.5 py-1.5 rounded-lg text-xs font-medium truncate"
            style={{
              background: "oklch(var(--portal-driver) / 0.18)",
              color: "oklch(var(--portal-driver-light))",
            }}
          >
            {driverName ?? schoolName}
          </div>
              ...
              {section.items.map(({ id, label, icon: Icon }) => {
                ...
                return (
                  <button
                    key={id}
                    type="button"
                    data-ocid={`driver.${id.replace(/-/g, "_")}.link`}
                    onClick={() => {
                      onPageChange(id);
                      setSidebarOpen(false);
                    }}
                    className="flex items-center gap-2.5 w-full text-left px-3 py-2 text-sm rounded-lg transition-all duration-150"
                    style={
                      isActive
                        ? {
                            background: "oklch(var(--portal-driver) / 0.2)",
                            color: "oklch(var(--portal-driver-light))",
                            fontWeight: 600,
                          }
                        : {
          ...
          {allNavItems.map(({ id, label, icon: Icon }) => (
            <button
              ...
              style={
                currentPage === id
                  ? {
                      background: "oklch(var(--portal-driver) / 0.12)",
                      color: "oklch(var(--portal-driver))",
                      fontWeight: 600,
                    }
                  : {}
